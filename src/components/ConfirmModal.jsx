...
const ConfirmModal = () => {
  const { confirmData, setConfirmData } = useAppStore();
  ...
  const onConfirm = (confirmed) => {
    window.electronAPI?.confirmTool(data.id, confirmed);
    setConfirmData(null);
  };
  ...
  return (
    <div 
      className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-[fadeSlideIn_0.15s_ease_both]"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div 
        ref={modalRef}
        className="w-full max-w-md bg-sidebar-bg border border-border-subtle rounded-2xl shadow-2xl p-6 flex flex-col gap-6"
      >
        {isGeneric ? (
          <>
            <div className="flex items-start gap-4">
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${data.isAlert ? 'bg-accent/20 text-accent' : 'bg-blue-500/20 text-blue-400'}`}>
                <AlertTriangle size={22} />
              </div>
              <div>
                <h2 id="modal-title" className="text-base font-semibold text-txt-primary">{data.title}</h2>
                <p className="text-sm text-txt-secondary mt-2 leading-relaxed">{data.message}</p>
              </div>
            </div>
          ...
          <>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <h2 id="modal-title" className="text-base font-semibold text-txt-primary">
